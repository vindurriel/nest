...
r.nest = function(options) {
  ...
  r.shiftPressed = false;
  r.blacklist = [];
  r.vis = d3.select(container).append("svg:svg").attr("viewBox", "0 0 " + r.w + " " + r.h).attr("pointer-events", "all").call(d3.behavior.zoom().scaleExtent([0.1, 10]).on("zoom", redraw)).on('dblclick.zoom', null).append("svg:g");
